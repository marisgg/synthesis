...
class SparseDerivativeInstantiationModelCheckerFamily {
   public:
    /**
     * Instantiates a new SparseDerivativeInstantiationModelCheckerFamily.
     * @param model The Dtmc to compute the derivatives of.
     */
    SparseDerivativeInstantiationModelCheckerFamily(storm::models::sparse::Dtmc<FunctionType> const& model) : model(model) {
        // Intentionally left empty.
    }
    virtual ~SparseDerivativeInstantiationModelCheckerFamily() = default;

    /**
     * specifyFormula specifies a CheckTask.
     * The SparseDerivativeInstantiationModelCheckerFamily will need to setup the matrices for it.
     ...
     */
    std::shared_ptr<modelchecker::ExplicitQuantitativeCheckResult<ConstantType>> check(
