...
class GradientDescentInstantiationSearcherFamily {
   public:
    /**
     * The GradientDescentInstantiationSearcherFamily can find extrema and feasible instantiations in pMCs,
     * for either rewards or probabilities.
     * Internally uses the SparseDerivativeInstantiationModelCheckerFamily to evaluate derivatives at instantiations.
     ...
     * SparseDerivativeInstantiationModelCheckerFamily.
    ...
    void setup(Environment const& env, std::shared_ptr<storm::pars::FeasibilitySynthesisTask const> const& task) {
        ...
        if (task->getFormula().isRewardOperatorFormula()) {
            ...
            if (rewardParameters.size() > 0) {
                STORM_PRINT_AND_LOG("WARNING: rewards parameters in model.");
                // exit(1);
            }
